...
var Q = _interopRequireWildcard(
  require('@nozbe/watermelondb/QueryDescription')
);
...
var getComparisonRight = function (table, comparisonRight) {
  if (comparisonRight.values) {
    ...
    return ''
      .concat((0, _encodeName.default)(table), '.')
      .concat((0, _encodeName.default)(comparisonRight.column));
  }

  return 'undefined' !== typeof comparisonRight.value
    ? (0, _encodeValue.default)(comparisonRight.value)
    : 'null';
...
var encodeComparison = function (table, comparison) {
  if ('between' === comparison.operator) {
    var { right: right } = comparison;
    return right.values
      ? 'between '
          .concat((0, _encodeValue.default)(right.values[0]), ' and ')
          .concat((0, _encodeValue.default)(right.values[1]))
      : '';
  }

  return ''
    .concat(operators[comparison.operator], ' ')
    .concat(getComparisonRight(table, comparison.right));
};
...
var encodeWhere = function (table, associations) {
  return function (where) {
    switch (where.type) {
      case 'and':
        return '('.concat(
          encodeAndOr(associations, 'and', table, where.conditions),
          ')'
        );

      case 'or':
        return '('.concat(
          encodeAndOr(associations, 'or', table, where.conditions),
          ')'
        );
      ...
      case 'on':
        (0, _common.invariant)(
          associations.some(function ({ to: to }) {
        ...
        return '('.concat(
          encodeAndOr(associations, 'and', where.table, where.conditions),
          ')'
        );
...
var encodeWhereCondition = function (associations, table, left, comparison) {
  ...
  return ''
    .concat((0, _encodeName.default)(table), '.')
    .concat((0, _encodeName.default)(left), ' ')
    .concat(encodeComparison(table, comparison));
};
...
var encodeAndOr = function (associations, op, table, conditions) {
  if (conditions.length) {
    return mapJoin(
      conditions,
      encodeWhere(table, associations),
      ' '.concat(op, ' ')
    );
  }
  ...
  if (countMode) {
    return needsDistinct
      ? 'select count(distinct '
          .concat((0, _encodeName.default)(table), '."id") as "count" from ')
          .concat((0, _encodeName.default)(table))
      : 'select count(*) as "count" from '.concat(
          (0, _encodeName.default)(table)
        );
  }

  return needsDistinct
    ? 'select distinct '
        .concat((0, _encodeName.default)(table), '.* from ')
        .concat((0, _encodeName.default)(table))
    : 'select '
        .concat((0, _encodeName.default)(table), '.* from ')
        .concat((0, _encodeName.default)(table));
...
var encodeAssociation = function (description) {
  return function ({ from: mainTable, to: joinedTable, info: association }) {
    ...
    var joinBeginning = ''
      .concat(joinKeyword)
      .concat((0, _encodeName.default)(joinedTable), ' on ')
      .concat((0, _encodeName.default)(joinedTable), '.');
    return 'belongs_to' === association.type
      ? ''
          .concat(joinBeginning, '"id" = ')
          .concat((0, _encodeName.default)(mainTable), '.')
          .concat((0, _encodeName.default)(association.key))
      : ''
          .concat(joinBeginning)
          .concat((0, _encodeName.default)(association.foreignKey), ' = ')
          .concat((0, _encodeName.default)(mainTable), '."id"');
  };
};

var encodeJoin = function (description, associations) {
  return associations.length
    ? associations.map(encodeAssociation(description)).join('')
    : '';
};
...
var encodeOrderBy = function (table, sortBys) {
  ...
  var orderBys = sortBys
    .map(function (sortBy) {
      return ''
        .concat((0, _encodeName.default)(table), '.')
        .concat((0, _encodeName.default)(sortBy.sortColumn), ' ')
        .concat(sortBy.sortOrder);
    })
    .join(', ');
...
var encodeQuery = function (query, countMode = false) {
  ...
  var hasToManyJoins = associations.some(function ({ info: info }) {
  ...
  (0, _common.invariant)(
    !description.lokiFilter,
    'unsafeLokiFilter not supported with SQLite'
  );
  var sql =
