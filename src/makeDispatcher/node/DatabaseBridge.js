...
var DatabaseBridge = function () {
  ...
  this.initialize = function (
    ...
    try {
      ...
      driver.initialize(databaseName, schemaVersion, client);
  ...
  this.setUpWithMigrations = function (
    tag,
    databaseName,
    migrations,
    fromVersion,
  ...
  this.initializeSynchronous = function (
    tag,
    databaseName,
    schemaVersion,
    client
  ...
  this.setUpWithSchemaSynchronous = function (
    tag,
    databaseName,
    schema,
    schemaVersion
  ...
  this.setUpWithMigrationsSynchronous = function (
    tag,
    databaseName,
    migrations,
    fromVersion,
  ...
  this.unsafeResetDatabase = function (
    tag,
    schema,
    schemaVersion,
    resolve,
  ...
  this.findSynchronous = function (tag, table, id) {
    return _this.withDriverSynchronous(
      tag,
      'findSynchronous',
      function (driver) {
        return driver.find(table, id);
    ...
    try {
      ...
      if ('connected' === connection.status) {
        if (connection.synchronous) {
          throw new Error(
            "Can't perform async action on synchronous connection ".concat(tag)
          );
        }
  ...
  this.connectDriverAsync = function (tag, driver) {
    var { queue = [] } = _this.connections[tag];
    _this.connections[tag] = {
      driver: driver,
      synchronous: false,
      queue: [],
  ...
  this.sendReject = function (reject, error, functionName) {
    if (reject) {
      ...
      throw new Error(
        'db.'
          .concat(functionName, ' missing reject (')
